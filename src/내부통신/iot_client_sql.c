...
int main(int argc, char* argv[]) {
    ...
    if (argc != 4) {
        printf("Usage : %s <Primary_IP> <Primary_Port> <ID>\n", argv[0]);
        exit(1);
    }
    ...
    pthread_create(&rcv_thread, NULL, recv_msg, (void*)&primary_sock);
    pthread_create(&snd_thread, NULL, send_msg, (void*)&primary_sock);
