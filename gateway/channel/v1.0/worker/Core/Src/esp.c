...
//    }
    ...
    if (huart->Instance == USART6) {
        if (cb_data.length < MAX_ESP_RX_BUFFER) {
            cb_data.buf[cb_data.length++] = data;  // 수신한 데이터 버퍼에 저장
        }
        // 데이터 처리 후 버퍼를 초기화
        cb_data.length = 0;

        // PDU 크기만큼 데이터가 수신되었으면 역직렬화
        //if (cb_data.length >= sizeof(RCT_Instance)) {
            // PDU 데이터가 완전히 수신된 경우 역직렬화 진행
            //deserialize_rct_data(cb_data.buf, &rct);

            // 수신된 데이터 확인 (디버깅용)
//            printf("Received Proto Ver: %s\n", pdu.rcp_header.proto_ver);
//            printf("Received Msg Type: %s\n", pdu.rcp_header.msg_type);
//            printf("Yaw: %.2f, Pitch: %.2f, Roll: %.2f\n", pdu.rc_data.yaw, pdu.rc_data.pitch, pdu.rc_data.roll);
//            printf("X Pos: %.2f, Y Pos: %.2f, Z Pos: %.2f\n", pdu.rc_data.x_pos, pdu.rc_data.y_pos, pdu.rc_data.z_pos);
//            printf("Color: %d\n", pdu.rc_data.color);
              //test_rcp(&rct);
        //}

        // UART6에서 계속 데이터를 수신할 수 있도록 재호출
        HAL_UART_Receive_IT(huart, &data, 1);
    }
}

// UART6로 데이터를 수신하는 함수
int uart_receive(UART_HandleTypeDef *huart, void *buffer, uint16_t size) {
...
//	version_func();
	...
	printf("init 1. wifi connect success!\n");
//	start_esp_server();
//	ip_state_func();
	request_ip_addr(1);
	printf("init 2 ip request success!\n");

	esp_client_conn();
	printf("init 3 server connect success!\n");
//	ip_state_func();
}
