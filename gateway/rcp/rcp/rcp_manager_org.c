...
// 직렬화 함수
size_t serialize(rct_instance *__src_rct_pdu, const uint8_t *__dst_buffer) {
...
void send_command(int client_fd, rct_instance ___cmd) {
    rct_instance __cmd;
    uint8_t buffer[DATA_SIZE];
    memset(&__cmd, 0, sizeof(rct_instance));

    static float yaw = 135.0, pitch = -42.0, roll = 50.0;
    static float pos_x = 320.0, pos_y = 500.0, pos_z = 150.0;

    size_t result = (size_t)snprintf(__cmd.rc_data.data, sizeof(__cmd.rc_data.data),
                 "Position: (%.2f, %.2f, %.2f)", pos_x, pos_y, pos_z) >= sizeof(__cmd.rc_data.data);
    if (result >= sizeof(__cmd.rc_data.data))
        fprintf(stderr, "Payload data truncated.\n");

    // 헤더 추가
    strncpy(__cmd.rcp_header.proto_ver, "rcp/1.1", sizeof(__cmd.rcp_header.proto_ver) - 1);
    __cmd.rcp_header.proto_ver[sizeof(__cmd.rcp_header.proto_ver) - 1] = '\0';
    strncpy(__cmd.rcp_header.msg_type, "COMMAND", sizeof(__cmd.rcp_header.msg_type) - 1);
    __cmd.rcp_header.err = RESPONSE_CODE_MANAGER_SUCCESS_TRANMIT;
    __cmd.rcp_header.d_len = strlen(__cmd.rc_data.data);
    __cmd.rcp_header.timestamp = time(NULL);

    // 직렬화
    size_t serialized_size = serialize(&__cmd, buffer);
    if ((size_t)write(client_fd, buffer, serialized_size) != serialized_size) {
        perror("write failed");
        return;
    }
...
uint8_t auth_login(struct sockaddr_in clnt_adr, int clnt_sock, char idpasswd[], int idpw_size) {
    int i;
    char *pToken;
...
void *conveyer_handler(void *arg) {
    ...
    while (1) {
        ...
        //if(task.machine_type == MACHINE_TYPE_CONVEYER) {
            ...
            //uint8_t task_buffer[4] = {client_fd, MACHINE_TYPE_ROBOT, detect_flag, color};
            ...
            if (mq_send(mq, (const char *)&task, sizeof(client_task_t), 0) == -1) {
                ...
                if (write(client_fd, "receive successfully", strlen("receive successfully")) == -1) {
                    ...
                    printf("Write to client B Success\n");
                }
            }
        ///}
    }
...
void *robot_handler(void *arg) {
    ...
    while (1) {
        ...
        printf("Robot thread is receive sucessfully.\n");
        printf("%d, %d, %d, %d\n", task.client_fd, task.machine_type, task.detect_flag, task.color);
    ...
    while (1) {
        ...
        for (int i = 0; i < nfds; i++) {
            ...
            if (events[i].data.fd == server_fd) {
                ...
                idpasswd[str_len] = '\0';
                machine_type = auth_login(client_addr, client_fd, idpasswd, str_len);
